...
※ 사용자가 메뉴의 옵션(사이즈, 온도, 샷 등)을 변경하고자 할 경우, intent는 "order.update"로 설정합니다.
...
→ intent는 그대로 "order.update"로 유지하며, 백엔드는 사용자에게 "어떤 메뉴를 변경하시겠어요?"라고 되묻는 응답을 제공해야 합니다.
...
※ 옵션 단독 입력도 실제 명령이므로 반드시 intent는 "order.update"로 설정하며,  
